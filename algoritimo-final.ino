...
const unsigned long debounce_interrupcao = 400;
...
void loop()
{
  ...
  switch(estado) {
    ...
    case CURVA:        
        debug("SWITCH"); 
        realizar_curva_esquerda(); 
        break;
    case PARADA:       
        desligar_ambos_motores(); 
        break;
  }
}
...
void interpretar_obstaculo() {
  if (leitura_obstaculo == LOW) {
    estado = PARADA;
  } else if(leitura_obstaculo == HIGH && em_curva == LOW) {
    estado = SEGUIR_LINHA;
  }
}
...
void loop_curva() {
    ...
    int sensor_guia = 0;
    while(em_curva == HIGH) {
        sensor_guia = digitalRead(A5); 
        if(sensor_guia == HIGH) {
            em_curva = LOW;
        }        
      delay(10);
    }
    debug("TERMINEI A CURVA");
}
